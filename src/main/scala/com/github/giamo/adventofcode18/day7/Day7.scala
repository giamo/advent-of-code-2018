...
object Day7 extends App {
  ...
  val NumWorkers    = 5
  val BaseStepDelay = 60

  val ordering = Ordering[Char].reverse // alphabetical order
  ...
  def part2(input: Seq[String]): Int = {
    val deps = parseInstructions(input)
    computeTotalDelay(deps, NumWorkers)
  }
  ...
  private def computeTotalDelay(initalDeps: Map[Char, Set[Char]], numWorkers: Int) = {
    ...
    def assignStepToWorker(workerId: Int) =
      if (stepsReadyQueue.nonEmpty) {
    ...
    @tailrec
    def computeTotalDelayRec(workers: Seq[Worker],
      ...
      if (updatedDeps.isEmpty && areAllWorkersIdle(updatedWorkers)) totalDelay
      else {
    ...
    val initialWorkers = (0 until numWorkers).map(Worker(_, None))
  ...
  private def parseInstructions(lines: Seq[String]) = {
    ...
    @tailrec
    def parseInstructionsRec(list: Seq[String], deps: Map[Char, Set[Char]]): Map[Char, Set[Char]] =
      list match {
        case Seq() => deps
        case Seq(l, tail @ _*) =>
          val Pattern(t, f)   = l
  ...
  private def updatedDependenciesMulti(deps: Map[Char, Set[Char]], completed: Set[Char]) =
    completed.foldLeft(deps) { (incr, c) =>
      updatedDependencies(incr, c)
    }

  private def stepDelay(id: Char) = BaseStepDelay + charToNumber(id)
